<template>
    <div class="container mt-4">
        <div class="d-flex justify-content-center">
            <h2 class="mb-4">Delivery Account Documentation Per Beneficiary</h2>
        </div>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select name="" id="" class="form-control" v-model="dataValues.district_id">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control" v-model="dataValues.beneficiary_id">
                                <option value="">Select Beneficiary</option>
                                <option v-for="beneficiary in beneficiaries" :value="beneficiary.id">{{ beneficiary.name }}</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-3">
...
export default {
    props: ['from_date', 'to_date', 'districts', 'district_id', 'project_id'],
        ...
        methods: {
            messageBox(title, text, type, reload) {
                this.$fire({
                    title : title,
                    text : text,
                    type : type,
            ...
            getProjects(){
                this.dataValues.project_id = '';
                this.projects = [];
                this.dataValues.beneficiary_id = '';
                this.beneficiaries = [];
                if (!this.dataValues.district_id) {
                    this.tableData = [];
                    return;
                }
                axios.get(`view_beneficiary/gather_projects/${this.dataValues.district_id}`)
                .then(response => {
            ...
            getBeneficiaries() {
                ...
                axios.get(`view_beneficiary/gather_beneficiaries/${this.dataValues.project_id}`)
                    .then(response => {
            ...
            gatherData(){
                this.isLoading = true;
                // Only gather if required fields are set
                if (!this.dataValues.project_id || !this.dataValues.date_from || !this.dataValues.date_to) {
                    this.tableData = [];
                    return;
                }
                axios.post('view_beneficiary/gather_data', this.dataValues)
                .then(response => {
                    this.tableData = response.data.data;
                    this.errors = {};
                })
                .catch(error => {
                    this.errors = error.response && error.response.data && error.response.data.errors ? error.response.data.errors : {};
        ...
        watch: {
            'dataValues.district_id'(val) {
                this.getProjects();
            },
            'dataValues.project_id'(val) {
